...
class Monster:
    ...
    def attack(self, target):
        ...
        if random.random() < self.effect_probability:
            ...
            # 应用状态
            status = CreateStatusByName(effect, duration=duration, target=target)
    ...
    def stun(self, duration: int) -> None:
        """使怪物晕眩"""
        self.statuses[StatusName.STUN] = CreateStatusByName(StatusName.STUN, duration=duration, target=self)
